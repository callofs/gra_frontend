  ...
  <section class="follow-panel card">
    <div class="panel-header">
      <div>
        ...
        <div class="panel-subtitle">查看关注你的用户列表。</div>
      </div>
    </div>
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card">
        <div class="stat-label">最新粉丝</div>
        <div class="stat-value">{{ users[0]?.nickname || users[0]?.username || '暂无' }}</div>
      </div>
    </div>

    <div v-loading="loading" class="list-wrap">
      <div v-if="!loading && users.length === 0" class="empty-state">你还没有粉丝</div>
...
async function fetchFollowers() {
  ...
  try {
    ...
    users.value = []
    ElMessage.error(error?.message || '获取我的粉丝失败')
  } finally {
    loading.value = false
  }
}
